...
export default function Home() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const throttleId = scrollThrottleRef.current
      if (throttleId) {
  ...
  return (
    <main className="relative h-screen w-full overflow-hidden bg-background">
      ...
      >
        ...
        >
          <div className="flex h-20 w-20 items-center justify-center rounded-lg bg-foreground/15 backdrop-blur-md transition-all duration-300 hover:scale-110 hover:bg-foreground/25">
            <img
              src="/drone-logo-perm.png"
              alt="IndiDriftX Logo"
              className="h-12 w-12"
            />
          </div>
          <span className="font-sans text-xl font-semibold tracking-tight text-foreground">IndiDriftX</span>
        </button>

        <div className="hidden items-center gap-8 md:flex">
          {["Home", "Work", "Services", "About", "Team", "Contact"].map((item, index) => (
            <button
              key={item}
          ...
          <div className="max-w-4xl">
            ...
            <p className="mb-8 max-w-xl animate-in fade-in slide-in-from-bottom-4 text-lg leading-relaxed text-foreground/90 duration-1000 delay-200 md:text-xl">
              <span className="text-pretty">
                Revolutionizing last-mile logistics with AI-driven autonomous drone swarms for rapid, reliable, and precise delivery.
              </span>
            </p>
          </div>
